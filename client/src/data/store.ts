...
export const selectFrameColor = (color: ColorName) => {
    useStore.setState({ selectedFrameColor: color,
        customFrameOutlineColor: "",
        customFrameFillColor: "",
    });
}
...
export const selectCenterColor = (color: ColorName) => {
    useStore.setState({ selectedCenterColor: color,
        customCenterOutlineColor: "",
        customCenterFillColor: "",
    });
}
...
export const selectModifierColor = (color: ColorName) => {
    useStore.setState({ selectedModifierColor: color,
        customModifierOutlineColor: "",
        customModifierFillColor: "",
    });
}
...
const processColorOverride = (mode: "outline" | "fill", part: "frame" | "center" | "modifier") => {
return (custom2: string, custom: ColorName | undefined, palette: PaletteName): string => {
    if (custom2) {
...
return (customOutline: string, customFill: string, custom: ColorName | undefined, palette: PaletteName): string => {
...
export const isPaletteCustom = createSelector(
    ...
    (frameCustom, centerCustom, modifierCustom, frame, center, modifier) => {
        return frameCustom || centerCustom || modifierCustom || !!(frame || center || modifier);
    }
);

const createIconUrl = (icon: string, modifier: string, fo: string, ff: string, co: string, cf: string, mo: string, mf: string) => {
        // remove #
            fo = fo.substring(1);
        ff = ff.substring(1);
        co = co.substring(1);
...
export const getIconUrlCreator = createSelector(
    ...
    return (icon: string) => createIconUrl(icon, modifier, fo, ff, co, cf, mo, mf);
});
